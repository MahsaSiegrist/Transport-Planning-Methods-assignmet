...
ggplot(p_hh, aes(y=N_O_HHM, x="")) +
  geom_boxplot(alpha=0.4, na.rm=TRUE) +
  theme_bw() +
  theme(legend.title=element_blank(),legend.position="bottom") + 
  xlab("X") +
  ylab("Household size")
ggsave(file = here::here("plots","household size.pdf"), width = 20, height = 15, units = "cm")
...
ggplot(p_hh, aes(y=AGE, x=sex_ch,fill=sex_ch)) +
  geom_boxplot(alpha=0.4, na.rm=TRUE) +
  theme_bw() +
  theme(legend.title=element_blank(),legend.position="bottom") + 
  xlab("Sex") +
...
as.data.frame(table(p_hh[,c("educ_f","employed_f")]))
...
ggsave(file = here::here("plots","educlevels_by_employstatus2.pdf"), width = 20, height = 15, units = "cm")
...
trip %>%
  ...
  xlab("Weekday")
ggsave(file = here::here("plots","nroftrips_per_weekday.pdf"), width = 20, height = 15, units = "cm")
...
ggsave(file = here::here("plots","nroftrips_per_income.pdf"), width = 20, height = 15, units = "cm")
...
ggsave(file = here::here("plots","totdist_per_income.pdf"), width = 20, height = 15, units = "cm")
...
trip_p_hh %>%
  ...
  ylab("Modal split") + xlab("Weekday") + labs(fill = "Main mode by time:") +
  scale_y_continuous(breaks = c(seq(0,1, by = 0.2))) +
  ggtitle("Mode choice by time per weekday")
ggsave(file = here::here("plots","modechoicetime_weekday.pdf"), width = 20, height = 15, units = "cm")
...
str(trips$T_DEP) # no value labels in original trips df
...
# exclude weekend
#d.weekday_tr<-subset(d.tr, weekday !="Sonntag" & weekday!="Samstag")
